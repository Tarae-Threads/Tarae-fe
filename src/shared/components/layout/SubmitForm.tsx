...
/* ---- 공통 장소 필드 (신규/업데이트 공용) ---- */
function PlaceDetailFields({ register, errors }: {
  ...
  return (
    <div className="flex flex-1 flex-col min-h-0">
      ...
      {submitted ? (
        ...
        placeMode === 'new' ? (
          <form onSubmit={placeForm.handleSubmit(onPlaceSubmit)} className="flex flex-1 flex-col min-h-0">
            {/* Place Mode Toggle */}
            <div className="flex gap-2 shrink-0 mb-4">
              <button type="button" onClick={() => setPlaceMode('new')}
                className="flex-1 py-2 text-label-md font-bold rounded-xl transition-all bg-secondary text-secondary-foreground">
                새 장소
              </button>
              <button type="button" onClick={() => setPlaceMode('update')}
                className="flex-1 py-2 text-label-md font-bold rounded-xl transition-all bg-surface-container text-on-surface-variant">
                기존 장소 업데이트
              </button>
            </div>
          ...
          <form onSubmit={updateForm.handleSubmit(onPlaceUpdate)} className="flex flex-1 flex-col min-h-0">
            {/* Place Mode Toggle */}
            <div className="flex gap-2 shrink-0 mb-4">
              <button type="button" onClick={() => setPlaceMode('new')}
                className="flex-1 py-2 text-label-md font-bold rounded-xl transition-all bg-surface-container text-on-surface-variant">
        ...
        <form onSubmit={eventForm.handleSubmit(onEventSubmit)} className="flex flex-1 flex-col min-h-0">
          {/* Scrollable fields */}
          <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar space-y-4 pb-2">
